...
public class VisualEntity extends Territory
{
    ...
    protected void initialize(String name, Node node, Spatial model, Vector3f position)
    {
        ...
        {
            if (b.getType() == BoundingVolume.Type.Sphere)
            {
    ...
    protected String[] setDisplayInfo()
    {
        ...
        if (superTerr != null)
        {
            for (String str : superTerr.getDisplayInfo())
            {
                infos.add(str);
            }
        }
    ...
    private void attachBoundingSphereToDrawable()
    {
        drawableObject.getModel().setUserData("Bounding", boundingSphere);
    }
}
